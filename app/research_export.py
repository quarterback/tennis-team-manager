...
def build_jhsaa(year: int, gender: str, classification: str = "all", *, season=None) -> dict[str, bytes]:
    ...
    manifest = {
        ...
        "domain_rules": ["JHSAA gender values are girls/boys (college uses women/men).",
            ...
            "for comparability, not places — and each Region is a QUALIFYING draw only: it "
            "runs down to four and stops, crowning nobody, so there is no regional champion, "
            "final or runner-up. Its 30 x 4 = 120 qualifiers seed a 128-slot State draw (eight "
            "byes) whose rounds carry debate parlance in round_names: R120, Triple Octafinals, "
            "Doubles Octafinals, Octafinals, Quarterfinals, Semifinals, Final. There are no "
            "at-large bids and no selection index. EARLIER seasons: the 36-team shape is twenty "
            "regional champions plus sixteen at-large selections (an index of 30% JV record + "
            "70% varsity regular-season record) playing a 'Parastate' first round for eight "
            "seats in a 28-team main draw; before that, twenty champions alone. "
            "jhsaa_jv_state.csv is that field FLAT for analysis, ONE schema across all three: "
            "one row per team in seed order — entry (qualifier from 2096; champion/at_large "
            "before), region, jv_* and varsity_reg_* records and percentages, selection_index "
            "(empty from 2096 — nothing is selected; the varsity columns are context, not a "
            "criterion), made_main_draw, state_place (teams alive when eliminated, 1 = "
            "champion) and state_finish — read off the same archived bracket. Seasons played "
            "at twenty list their champions with the index columns empty. "
