...
def prior_meetings(league_id: int, fid_a: int, fid_b: int, exclude_dual_id: int) -> list[dict]:
    ...
    `label` is a real calendar date: `year` runs concurrent with the college
    calendar (`BASE_YEAR + year`, this module's own convention — see the
    comment above `BASE_YEAR`), so unlike the raw index a bare "Wk N" showed,
    the YEAR here is real. The day-within-year is still synthetic (the pro
    league has no per-dual calendar the way JHSAA does) — the SAME
    week-offset convention `season_schedule`'s college page already uses,
    just anchored to this season's real year instead of a fixed one."""
    import datetime
    conn = _db()
    rows = conn.execute(
