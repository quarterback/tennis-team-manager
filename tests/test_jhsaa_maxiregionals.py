"""THE MAXIREGIONALS — the at-larges' first qualifying layer (owner rule 2026-09).

The Parastate put the `2 x bids` lowest seeds together, so a 48's seeds 17-24 drew
seeds 41-48 and won 279 of 288 of those duals over six seasons. The maxis pair the
...
* a maxi loser is NOT a State participant, which is the one substantive break
  with the Parastate (where every entrant is, because the Parastate is a round OF
  the State event). That is bought by the maxis being their own PHASE with their
...
def _play(group, n_field):
    ...
    return jh.run_state_parastate(field, byes=jh.parastate_byes(group),
                                  maxi=jh.maxi_bids(group), seed=11)


# --------------------------------------------------------------- the tables ----

def test_1a_and_group_3_do_not_play_the_maxis():
    ...
    for g in ("1A", "Group 3"):
        assert g in jh.ATLARGE_GROUPS and g not in jh.MAXI_GROUPS
        assert jh.maxi_bids(g) == 0
...
def test_every_maxi_class_is_an_at_large_class_with_an_even_allocation():
...
def test_the_arithmetic_closes_on_the_road_field_at_every_shape():
    ...
    maxis take `bids` and return `bids / 2`; the Parastate takes those plus
    ...
    for g in jh.ATLARGE_GROUPS:
        ...
        through_maxi = jh.maxi_bids(g) // 2
        post_maxi = crowns_from - through_maxi          # losers are out
        para = post_maxi - jh.parastate_byes(g)
        assert para % 2 == 0, g
...
def test_the_maxis_pair_the_lowest_seeds_among_themselves(group, n_field, duals):
    arc = _play(group, n_field)
    maxi = arc["maxi"]
    bids = jh.AT_LARGE_BIDS[group]
    # Exactly the bottom `bids` seeds, and nobody else.
    assert maxi["field"] == [f"T{i:02}" for i in range(n_field - bids + 1, n_field + 1)]
    assert len(maxi["rounds"][0]) == duals
    assert maxi["round_names"] == [jh.MAXI_NAME]
    # Paired high-low WITHIN the block — the best maxi seed meets the worst.
    first = maxi["rounds"][0][0]
    assert {first["home"], first["away"]} == {f"T{n_field - bids + 1:02}",
                                              f"T{n_field:02}"}


@pytest.mark.parametrize("group, n_field", [("9A", 48), ("7A", 40)])
def test_a_maxi_loser_is_not_in_the_state_field(group, n_field):
    """‼️ THE WHOLE POINT. `world.jhsaa_state_result` reports `made_state` off
    membership of the draw's `field`, so keeping a maxi loser out of that list is
    the only thing that makes 'a Maxiregional participant with no State
    appearance' true."""
    arc = _play(group, n_field)
    bids = jh.AT_LARGE_BIDS[group]
    through = set(arc["maxi"]["advanced"])
    played = set(arc["maxi"]["field"])
...
def test_the_state_draw_is_the_road_field_and_the_top_seeds_bye_it(group, n_field):
    arc = _play(group, n_field)
    assert len(arc["field"]) == n_field - jh.maxi_bids(group) // 2
...
def test_a_maxi_winner_keeps_its_seed():
    ...
    advanced = [n for n in arc["maxi"]["advanced"]]
...
def test_the_classes_that_do_not_play_them_are_byte_identical():
    """1A passes `maxi=0` and its old byes, so its draw must be exactly what it
    ...
    new = jh.run_state_parastate(field, byes=jh.parastate_byes("1A"),
                                 maxi=jh.maxi_bids("1A"), seed=11)
    old = jh.run_state_parastate(field, byes=32 - 8, seed=11)
    assert "maxi" not in new
...
def test_a_short_field_plays_no_maxis_at_all():
    """‼️ THE MAXIS NEVER EAT INTO THE BYE LINES. Capped with `min(maxi, len(field))`
    a small world (a fixture's two districts a class, a save whose road ran dry)
    put its WHOLE field into the maxis: every team played one, half were out
    ...
    degrades exactly as it did before the maxis existed."""
    for n in (6, 12, 24, 32, 39):
        ...
        arc = jh.run_state_parastate(field, byes=jh.parastate_byes("9A"),
                                     maxi=jh.maxi_bids("9A"), seed=5)
        # 9A needs 24 bye lines plus a 16-team maxi block: 40 teams.
        assert "maxi" not in arc, n
        assert arc["field"] == [t.school.name for t in field], n
        # ‼️ AND THE PARASTATE TAKES THE WHOLE ALLOCATION BACK, so a short world
        # still PLAYS one. Passing `road − bids / 2` with no maxis behind it left
        # the bye line too high by the half the maxis would have removed, and the
    ...
    arc = jh.run_state_parastate(field, byes=jh.parastate_byes("9A"),
                                 maxi=jh.maxi_bids("9A"), seed=5)
    assert "maxi" in arc and len(arc["maxi"]["rounds"][0]) == 8


# ---------------------------------------------------------------- the finish ----

def test_a_maxi_exit_reads_back_as_its_own_finish_and_supersedes_the_specials():
    """A Specials WINNER is a State qualifier and can be seeded into the maxis, so
    the maxi check has to be tried before the Specials or its year would read as
    ending a round earlier than it did."""
    grp = {jh.MAXI_PHASE: {"field": ["Alpha", "Beta"]},
           "state_special": {"field": ["Alpha"]},
           "state": {"field": ["Gamma"], "rounds": [], "champion": "Gamma"}}
    res = wd.jhsaa_postseason_result(grp, "Alpha")
    assert res["finish"] == jh.MAXI_FINISH and res["made_state"] is False
    assert jh.MAXI_FINISH == "Maxiregionals"
    # And it ranks deeper than a Specials exit on the season-depth ladder.
    ladder = wd.jh_road_ladder()
    assert ladder.index(jh.MAXI_FINISH) > ladder.index(jh.STATE_SPECIAL_FINISH)


def test_a_season_archived_before_the_maxis_still_reads():
...
def test_the_maxis_render_as_their_own_stage_on_the_bracket_page(monkeypatch):
    ...
    small world where the maxis correctly do not convene at all (a field under
    ...
    grp = jh.MAXI_GROUPS[0]
    maxi = {"field": ["Alpha", "Beta", "Gamma", "Delta"],
            "round_names": [jh.MAXI_NAME],
            "advanced": ["Alpha", "Beta"],
