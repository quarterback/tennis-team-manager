...
def test_jhsaa_bundle_marks_the_team_captains():
    """Captains were UI-only (the roster chip on the program page); the export
    now carries them so an analyst can read them off players.csv."""
    from app.jhsaa import GROUPS
    ...
    assert rows["cat"]["captain"] == "1" and rows["cat"]["captain_order"] == "1"
    assert rows["ana"]["captain"] == "1" and rows["ana"]["captain_order"] == "2"
