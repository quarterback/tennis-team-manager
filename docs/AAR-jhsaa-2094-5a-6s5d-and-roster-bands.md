# AAR — 5A's 6S/5D card, and the roster bands re-cut for the JV ladder (JHSAA rule 2094)

Two changes, one rule, because one of them forced the other.

**5A plays 6 singles / 5 doubles** — eleven courts, **sixteen on court** — on its road to
State, in the State draw and in a 5A-hosted showcase. It is the widest card in the
association by two courts and the only one where singles outweigh doubles.

**`ROSTER_FLOOR` goes 16 → 20 and every band is re-cut**, because the JV ladder had
outgrown bands that were never resized for it, and because sixteen on court against a
...
programs carry enough players to fill a card that size — and that objection did not
survive the exports.
...
| Sides that failed to fill the card | **0** |

So the capacity objection was already falsified by play the association had been running
for years: under "the wider card wins" a 5A team drawn against a wide-class showcase host
simply plays nine courts, and they always filled it. What 6S/5D asks that 4S/5D does not
...
did not ask for more courts; it asked to be **the singles class**. At 6/11 courts this is
the only JHSAA shape where singles outweigh doubles — every other postseason card runs 20%
...
`FLIGHT_WEIGHTS_6S5D` carries that through to what a court is WORTH: **S1 is priced above
D1** (2.00 vs 1.50) where every other table in the association ties them, and the six
singles seats carry 65% of the card's weight. A class that wanted width alone would have
...
Eleven courts is odd, so a 6S/5D dual **cannot tie** and no tie-breaking logic is
...
generic per-slot reader ranks a sixth singles court below a fifth instead of taking the
...
the three-court minimum, and **three bands had minima BELOW the floor** (1A 14, Group 3 14,
2A 15) — dead numbers the floor was silently overriding.

**5A's card now needs sixteen.** A floor-sized program would dress its entire roster with
...
**The regular season.** 5A's league card is still 3S/4D like everyone's, so `jv_pool` and
...
- Only ten of the 231 5A dual-sides at 4S/5D were 5A-versus-5A; the rest were against larger
  classes in showcases. For "can they fill the card" that does not matter.
