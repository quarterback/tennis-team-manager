...
from .state import Player, ATTRS
...
def _pair_player(team: Team, pair: tuple[int, int]) -> Player:
    ...
    attrs = {at: (getattr(a, at) + getattr(b, at)) / 2.0 for at in ATTRS}
    return Player(name=f"{a.name} / {b.name}", country=a.country, **attrs)


def simulate_dual(home: Team, away: Team, *, seed: int, fidelity: str = "full") -> DualResult:
    lines: list[DualLine] = []
    ...
    for i in range(3):
        ...
        res = simulate_match(hp, ap, seed=seed + 10 + i, fmt=doubles_pro, fidelity=fidelity)
