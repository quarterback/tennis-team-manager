...
format) and the two things it turned out to depend on: what the JV ladder needs from
...
def main(roots):
    ...
    print("=== Lineup size, and 5A headroom ===")
    print(f"{'format':18} {'flights':>7} {'players':>7} {'min spare':>10} {'median':>7} {'% short':>8}")
    for name, (S, D) in [("3S/2D", (3, 2)), ("2S/3D", (2, 3)), ("1S/4D (old)", (1, 4)),
                         ("3S/4D", (3, 4)), ("4S/4D", (4, 4)), ("3S/5D", (3, 5)),
                         ("4S/5D", (4, 5)), ("6S/5D (rule 2094)", (6, 5))]:
        on = S + 2 * D
        sp = [r - on for r in n5]
        print(f"{name:18} {S + D:7d} {on:7d} {min(sp):10d} {st.median(sp):7.1f} "
              f"{100 * sum(1 for x in sp if x < 0) / N:7.1f}%")
    print("\n  Even flight counts (4S/4D, 3S/5D) can tie; 1S/4D, 3S/4D, 4S/5D and 6S/5D cannot.")

    print("\n=== Has it been done? 5A dual-sides by shape as PLAYED ===")
    for (g, shape), c in sorted(fill.items()):
        if g != "5A" or c["sides"] < 5:
            continue
        print(f"  {shape:7} {c['sides']:6d} sides   failed to fill the format: {c['short']}")

    print("\n=== What the JV ladder needs from the bands ===")
    print(f"  JV pool = roster - {VARSITY_CUT} (the regular-season format; a road/State")
    print("  shape change does NOT move it). Smallest JV shape needs 5 a side.\n")
    print(f"{'class':9} {'n':>5} {'roster min':>11} {'median':>7} {'JV spare at min':>16} {'JV format':>12}")
    for g in ORDER:
        v = sorted(rosters[g])
        if not v:
            continue
